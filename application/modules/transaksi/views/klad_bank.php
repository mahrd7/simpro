<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            var store_kladbank = Ext.create('Ext.data.Store', {
                model: 'kladbank',
                remoteFilter: true,
                autoLoad: false,
                // groupField: 'subbidang_name',
                 proxy: {
                    ...
                    url: '<?php echo base_url() ?>transaksi/get_data_kladbank',
                    reader: {
                        type: 'json',
                        root: 'data'
                    }
            ...
            Ext.onReady(function() {
                ...
                var grid_kladbank = Ext.create('Ext.grid.Panel', {
                    ...
                    columns: [
                        ...
                        {text: "", xtype: 'actioncolumn', width: 25, sortable: true, icon: '<?= base_url(); ?>assets/images/delete.gif',
                            handler: function(grid, rowIndex, colIndex) {
                                ...
                                Ext.MessageBox.confirm('Delete', 'Apakah anda akan menghapus item ini?',function(resbtn){
                                    if (resbtn == 'yes') {
                                        Ext.Ajax.request({
                                            url: '<?=base_url();?>transaksi/delete_data/kladbank',
                                            method: 'POST',
                                            params: {
                                                'id':id
                                            },                              
                        ...
                        {text: "Kredit", width:120, sortable: true, dataIndex: 'jumlah',
                            renderer: Ext.util.Format.numberRenderer('Rp 00,000')
                        },
                            ...
                            items: [
                                ...
                                'S/D : ',
                                {
                                    ...
                                    handler: function(){
                                        ...
                                        store_kladbank.proxy.setExtraParam('sort', sort);
                                        store_kladbank.proxy.setExtraParam('pilihan_sort', pilihan_sort);
                                        store_kladbank.proxy.setExtraParam('tgl_awal', tgl_awal);
                                        store_kladbank.proxy.setExtraParam('tgl_akhir', tgl_akhir);
                                        store_kladbank.loadPage(1);
                                    }
                                }, '-', {
                                    text: 'Print'
                                }
            ...
            function tambahkladbank() {
                ...
                var frmtambahkladbank = Ext.create('Ext.form.Panel', {
                    // url: '<?php echo base_url() ?>transaksi/insertdata/kladbank',
                    ...
                    items: [                        
                        ...
                        {
                            xtype: 'radiogroup',
                            fieldLabel: 'Pilih Debet / Kredit',
                            itemId: 'status',
                            columns: 2,
                            items: [
                                {boxLabel: 'Kredit', name: 'status', itemId: 'kredit', inputValue: 'kredit'},
                                {boxLabel: 'Debet', name: 'status', itemId: 'debet', inputValue: 'debet'}
                            ],
                            listeners: {
                                change: function(val){
                                    if (val.getValue().status == 'kredit') {
                                        frmtambahkladbank.getComponent('jumlah').setVisible(false);
                                        frmtambahkladbank.getComponent('kode_toko').setVisible(true);
                                        frmtambahkladbank.getComponent('pilihan').setVisible(true);
                                        frmtambahkladbank.getComponent('button_tambah').setVisible(true);
                                        frmtambahkladbank.getComponent('button_delete').setVisible(true);
                                        frmtambahkladbank.getComponent('grid_1').setVisible(true);
                                    } else {
                                        frmtambahkladbank.getComponent('jumlah').setVisible(true);
                                        frmtambahkladbank.getComponent('kode_toko').setVisible(false);
                                        frmtambahkladbank.getComponent('pilihan').setVisible(false);
                                        frmtambahkladbank.getComponent('button_tambah').setVisible(false);
                                        frmtambahkladbank.getComponent('button_delete').setVisible(false);
                                        frmtambahkladbank.getComponent('grid_1').setVisible(false);
                                    }
                                }
                            }
                        },
                                ...
                                if (form.isValid()) {
                                    if (store_temp.getRange().length > 0) {
                                        for (var i = 0; i < store_temp.getRange().length; i++) {
                                            ...
                                            Ext.Ajax.request({
                                                url: '<?=base_url();?>transaksi/insertdata/kladbank',
                                                method: 'POST',
                                                params: {
                                                    'no_bukti':frmtambahkladbank.getComponent('no_bukti').getValue(),
                                                    'pic':frmtambahkladbank.getComponent('pic').getValue(),
                                                    'kode_toko':frmtambahkladbank.getComponent('kode_toko').getValue(),
                                                    'tanggal':frmtambahkladbank.getComponent('tanggal').getValue(),
                                                    'pilihan':frmtambahkladbank.getComponent('pilihan').getValue(),
                                                    'item':rec.get('id'),
                                                    'volume':rec.get('volume'),
                                                    'jumlah_bayar':rec.get('jumlah'),
                                                    'keterangan_item':rec.get('keterangan_item'),
                                                    'kode_rap':rec.get('kode_rap'),
                                                    'uraian':frmtambahkladbank.getComponent('uraian').getValue(),
                                                    'status':frmtambahkladbank.getComponent('status').getValue(),
                                                    'debet':frmtambahkladbank.getComponent('jumlah').getValue(),
                                        ...
                                        radio=frmtambahkladbank.getComponent('status');
                ...
                winadds = Ext.create('Ext.Window', {
                    ...
                    height: '80%',
                    layout: 'fit',
                    items: frmtambahkladbank
                }).show();
            }

            function tambah_item_kladbank() {
                        ...
                        items: [
                            {
                                ...
                                handler: function() {
                                    winadd.hide();
                                    var selectedNode = grid_kladbank.getSelectionModel().getSelection();
            ...
            function editcashtodate(id,urutan,tanggal,no_bukti,pic,uraian,pilih_toko_id,pilihan,volume,jumlah,keterangan_item,detail_material_id,debet,kode_rap) {
                ...
                var frmtambahcashtodate = Ext.create('Ext.form.Panel', {
                    url: '<?php echo base_url() ?>transaksi/editdata/kladbank',
