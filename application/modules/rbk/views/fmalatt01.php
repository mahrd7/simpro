...
<script type="text/javascript">
    ...
    Ext.define('mdl', {
        ...
        fields: [
            ...
            {name: 'spesifikasi_meterial', mapping: 'spesifikasi_meterial'},
    ...
    var storerrp = Ext.create('Ext.data.Store', {
        model: 'mdl',
        pageSize: 100,
        remoteFilter: true,
        autoLoad: false,      
        ...
        proxy: {
            ...
            url: '<?php echo base_url() ?>rbk/get_data_rrp',
            reader: {
                type: 'json',
                root: 'data'
            }
    ...
    Ext.onReady(function() {
        ...
        groups = storerrp.getGroups();

        var grid = Ext.create('Ext.grid.Panel', {
            title: 'Rincian rencana peralatan, kendaraan & Investasi Extra Contable (FM-RRP-T01)',
            store: storerrp,
            features: [groupingFeature], 
            plugins: [rowEditing],
            columns: [
                {text: "KODE MATERIAL", width:120, sortable: true, dataIndex: 'kode_meterial'},
                {text: "JENIS PERALATAN /<br>KENDARAAN", width:120, sortable: true, dataIndex: 'jenis_material'},
                {text: "SPESIFIKASI PERALATAN /<br>KENDARAAN", width:160, sortable: true, dataIndex: 'spesifikasi_meterial'},
            ...
            dockedItems: [{
                ...
                //         change:function(val){
                //             value = val.getValue();
                //             storerrp.proxy.setExtraParam('tgl_awal', value);
                //             storerrp.load();
                //         }
                //     }
                // },'-','s/d','-',{
                ...
                //         change:function(val){
                //             value = val.getValue();
                //             storerrp.proxy.setExtraParam('tgl_akhir', value);
                //             storerrp.load();
                //         }
                //     }
                // },'-',
                {
                    ...
                    listeners:{
                        change: function(val){
                            value = val.getValue();
                            storerrp.proxy.setExtraParam('sort', value);
                            storerrp.load();
                            // console.log(sort);
                        }
                    }
                },'-',{
                    text:'Export Rencana Penggunaan RRP',
                    iconCls:'icon-print',
                    handler:function(){
                        Ext.MessageBox.confirm('Export', 'Apakah anda akan meng-Export item ini?',function(resbtn){
                            if(resbtn == 'yes')
                            {
                                window.location='<?=base_url()?>rbk/print_data/rencana_penggunaan_rrp';                                                                             
                            }
                        });
                    }
                },'-',
