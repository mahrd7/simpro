...
<script type="text/javascript">
    ...
    Ext.onReady(function() {
		...
		var storeANSAT = Ext.create('Ext.data.Store', {
			 ...
			 proxy: {
				type: 'ajax',
				url: '<?=base_url();?>rbk/rbk_analisa/get_data_ansat/<?=$id_proyek;?>',
				reader: {
					type: 'json',
					root: 'data',
					totalProperty: 'total'
					...
					items: [
						{
							...
							handler: function(){          
									...
									{
										Ext.Ajax.request({
											url: '<?=base_url();?>rbk/rbk_analisa/tambah_apek',
											method: 'POST',											
											params: {																	
					...
					items: [
						{
							...
							handler: function(){
								...
								{
									Ext.Ajax.request({
										url: '<?=base_url();?>rbk/rbk_analisa/tambah_ansat',
										method: 'POST',
										params: {			
											/*
		...
		var winANSAT = Ext.widget('window', {
					...
					{text: "",xtype: 'actioncolumn', flex:1, align: 'center', sortable: true,icon:'<?=base_url();?>assets/images/delete.gif',
						handler: function(grid, rowIndex, colIndex){        
							...
							Ext.MessageBox.confirm('Delete item', 'Apakah anda akan menghapus item ini?',function(resbtn){
									...
									{
										Ext.Ajax.request({
											url: '<?=base_url();?>rbk/rbk_analisa/delete_asat/'+id,
											method: 'POST',
											params: {
												'id_analisa_asat':id,
						...
						items: [
							{
								...
								handler: function(){
									...
									{
										Ext.Ajax.request({
											url: '<?=base_url();?>rbk/rbk_analisa/copy_asat',
											method: 'POST',											
											params: {												
								...
								handler: function(){        
									...
									if(analisa != '')
									{
										...
										function(resbtn){
												...
												{
												Ext.Ajax.request({
													url: '<?=base_url();?>rbk/rbk_analisa/delete_ansat',
													method: 'POST',											
													params: {
			...
			var storeAnalisaPekerjaan = Ext.create('Ext.data.Store', {
				...
				proxy: {
					 type: 'ajax',
					 url: '<?php echo base_url() ?>rbk/rbk_analisa/get_daftar_analisa/<?=$id_proyek;?>',
					 reader: {
						 type: 'json',
						 root: 'data'
					 }
			...
			var APcellEditing = Ext.create('Ext.grid.plugin.RowEditing', {
					...
					listeners : {
						'edit' : function() {						
							...
							Ext.Ajax.request({
								url: '<?=base_url();?>rbk/rbk_analisa/tambah_daftar_analisa',
								method: 'POST',
								params: {
									'id_proyek' : <?=$id_proyek;?>,
			...
			var storeEditHargaASAT = Ext.create('Ext.data.Store', {
				...
				proxy: {
					 type: 'ajax',
					 url: '<?php echo base_url() ?>rbk/rbk_analisa/edit_harga_satuan_asat/<?=$id_proyek;?>',
					 reader: {
						 type: 'json',
						 root: 'data'
					 }
				...
				plugins: Ext.create('Ext.grid.plugin.RowEditing', {
					...
					listeners : {
						edit : function() {
							...
							Ext.Ajax.request({
								url: '<?=base_url();?>rbk/rbk_analisa/update_harga_asat',
								method: 'POST',
								params: {
									'id_proyek' : <?=$id_proyek;?>,
        ...
        var frmUploadAnalisa = Ext.widget({
			xtype: 'form',
			layout: 'form',
			url: '<?php echo base_url(); ?>rbk/rbk_analisa/upload_daftar_analisa',
			frame: false,
			bodyPadding: '5 5 0',
			width: 350,
			fieldDefaults: {
					...
					{text: "",xtype: 'actioncolumn', flex:1,  align: 'center', sortable: false,icon:'<?=base_url();?>assets/images/application_go.png',
						handler: function(grid, rowIndex, colIndex){        
							var rec = storeAnalisaPekerjaan.getAt(rowIndex);
								Ext.Ajax.request({
									url: '<?=base_url();?>rbk/rbk_analisa/set_analisa_itemid',
									method: 'POST',
									params: {
					...
					{text: "",xtype: 'actioncolumn', flex:1, align: 'center', tooltip: 'Delete item ini', sortable: true,icon:'<?=base_url();?>assets/images/delete.gif',
						handler: function(grid, rowIndex, colIndex){        
							...
							Ext.MessageBox.confirm('Delete item', 'Apakah anda akan menghapus item ('+item+') ini?',function(resbtn){
									...
									{
										Ext.Ajax.request({
											url: '<?=base_url();?>rbk/rbk_analisa/delete_analisa_pekerjaan/'+id,
											method: 'POST',
											params: {
												'kode_analisa':id,
												'id_proyek':<?=$id_proyek;?>,
					...
					{text: "",xtype: 'actioncolumn', flex:1, align: 'center', tooltip: 'Hapus seluruh analisa untuk item ini', sortable: true,icon:'<?=base_url();?>assets/images/cross.gif',
						handler: function(grid, rowIndex, colIndex){        
							...
							Ext.MessageBox.confirm('Delete item', 'Apakah anda akan menghapus seluruh Analisa Satuan Pekerjaan untuk item ('+item+') ini?',function(resbtn){
									...
									{
										Ext.Ajax.request({
											url: '<?=base_url();?>rbk/rbk_analisa/delete_asat_apek/',
											method: 'POST',
											params: {
												'kode_analisa':id,
												'id_proyek':<?=$id_proyek;?>,
						...
						items: [
							{
								...
								iconCls: 'icon-back',
								handler: function(){    
									window.location = '<?=base_url();?>rbk/edit_rap/<?=$id_proyek;?>';
								}
							},'-',
							{
								text:'Tambah Data',
							...
							},'-',
							{
								...
								handler: function(){          
									Ext.Ajax.request({
										url: '<?=base_url();?>rbk/rbk_analisa/clear_search_data_analisa',
										method: 'POST',
										params: {
											'clearsearch' : 1,
											'page' : 1,
						...
						items: [
							...
							
							{
								...
								handler: function(){          
									...
									{
										Ext.Ajax.request({
											url: '<?=base_url();?>rbk/rbk_analisa/paste_asat',
											method: 'POST',											
											params: {												
												'kode_analisa' : analisa.join(','),
								...
								handler: function(){          
									...
									if(analisa != '')
									{
										...
										function(resbtn){
												...
												{
												Ext.Ajax.request({
													url: '<?=base_url();?>rbk/rbk_analisa/delete_analisa',
													method: 'POST',											
													params: {												
														'kode_analisa' : analisa.join(','),
														'id_data_analisa': idanalisa.join(','),
	...
	Ext.define('mdlAnalisaPekerjaan2', {
		...
		fields: [
		'id_data_analisa', 'kode_analisa', 'id_kat_analisa', 'kategori', 'nama_kategori',
		'nama_item', 'id_satuan', 'satuan', 'id_proyek', 'harga_satuan', 'jenis_analisa', 'kd', 'xn'
		]
	});
	...
	var storeAnalisaPekerjaan2 = Ext.create('Ext.data.Store', {
		...
		proxy: {
			type: 'ajax',
			url: '<?php echo base_url() ?>rbk/rbk_analisa/get_data_analisa_pekerjaan_copy',
			reader: {
				type: 'json',
				root: 'data'
			}
				...
				dockedItems:[{
						...
						},
						{
							...
							store: { 
								...
								proxy: { 
									...
									type: 'ajax', 
									url: '<?=base_url();?>rbk/get_proyek_pilih', 
									reader: { 
										root: 'data',
										type: 'json' 
									} 
					...
					'->',
					{
						...
						handler:function(){
							...
							if (res == 'yes') {
								Ext.Ajax.request({
									url:'<?=base_url()?>rbk/copy_analisa_proyek_lain',
									method:'POST',
									params:{
										'id_data':id_data.join(','),
										'id_proyek':gridAnalisaPekerjaan.getDockedItems()[0].getComponent('proyek_pilih').getValue(),
										'jml':jml
									},
...
var winDaftarAnalisa = Ext.widget('window', {
	title: 'Data Analisa Pekerjaan :: Proyek -> <?=$data_proyek['proyek'];?>',
	closeAction: 'hide',
	width: '70%',
	height: '80%',
	layout: 'fit',
