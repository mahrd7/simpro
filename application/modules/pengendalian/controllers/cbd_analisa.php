...
class cbd_Analisa extends MX_Controller {
	...
	function __construct()
	{
		...
		if(!$this->session->userdata('logged_in'))
			redirect('main/login');
		$this->load->model('mdl_cbd_analisa');
	...
	public function get_asat($idproyek,$tgl_rab)
	{
		...
		$data = $this->mdl_cbd_analisa->get_daftar_analisa_satuan($idproyek,$tgl_rab);		
		$this->_out($data);
	}
	...
	function delete_asat()
	{
		if($this->input->post('id_analisa_asat'))
		{
			...
			{
				if($this->db->delete('simpro_current_budget_analisa_apek',array('id_analisa_apek'=>$this->input->post('id_analisa_asat'))))
					echo "Data berhasil dihapus.";
						else echo "Data GAGAL dihapus!";				
			} else
			{
				if($this->db->delete('simpro_current_budget_analisa_asat',array('id_analisa_asat'=>$this->input->post('id_analisa_asat'))))
					echo "Data berhasil dihapus.";
						else echo "Data GAGAL dihapus!";
			}
		}
	...
	{
		$data = $this->mdl_cbd_analisa->rat_rata($idproyek,$tgl_rab);
		$this->_out($data);		
	}
	...
	{	
		...
		)
		{
			if(substr($this->input->post('detail_material_kode'),0,2) <> 'AN')
			{
				...
				if($this->db->update('simpro_current_budget_analisa_asat', $update))
			...
			} else
			{
				...
				if($this->db->update('simpro_current_budget_analisa_apek', $update))
					echo "Data berhasil diupdate.";
						else echo "Data GAGAL diupdate!";
			}
		} 
	...
	function set_parent_tree_id()
	{
		...
		{		
			$datasess = array(
				...
				'current_budget_parent_tree_id' => $this->input->post('parent_id'),
				'current_budget_parent_kode_tree' => $this->input->post('parent_kode_tree')
			);
			$_SESSION['sess_proyek_id'] = $this->input->post('proyek_id');
			$_SESSION['sess_current_budget_parent_id'] = $this->input->post('parent_id');
			$_SESSION['sess_current_budget_parent_kode_tree'] = $this->input->post('parent_kode_tree');
			$this->session->set_userdata($datasess);
		} else
		{
			$datasess = array(
				'current_budget_parent_tree_id' => 0,
				'current_budget_parent_kode_tree' => ''
			);
			$_SESSION['sess_current_budget_parent_id'] = 0;
			$_SESSION['sess_current_budget_parent_kode_tree'] = '';
			$this->session->set_userdata($datasess);
		}
	}
	...
	{
		$parent = @$_SESSION['sess_current_budget_parent_kode_tree'];
		return $this->session->userdata('current_budget_parent_kode_tree') <> '' ? $this->session->userdata('current_budget_parent_kode_tree') : $parent;
	}
	
		
	public function tambah_current_budget_tree_item($idproyek,$tgl_rab)
	{		
		...
		*/
		...
		{				
			...
			$resin = $this->db->insert('simpro_current_budget_item_tree', $datainsert);
			if($resin)
			{
				echo json_encode(array('success'=>true, 'message'=>'Data current_budget berhasil disimpan!'));
	...
	{
		$data = $this->mdl_cbd_analisa->get_detailmaterial_kode($this->input->get('query'));
		$this->_out($data);	
	}
	...
	{		
		/*
			SELECT * FROM simpro_current_budget_item_tree
			WHERE id_proyek = '%d'
			ORDER BY kode_tree, current_budget_item_tree ASC		
		*/
		...
		if($this->input->post('kode_tree') && $this->input->post('id_proyek'))
		{
			if(isset($_SESSION['copy_tree']['data']))
			{
				...
				foreach($copy as $cp)
				{
					...
					if(!$this->db->insert("simpro_current_budget_item_tree", $data)) $error=TRUE;
					$i++;
				}
				//$this->_dump($data);
	...
	function tree($idpro, $depth='isnull',$tgl_rab,$param)
	{
		$result=array();
		$temp=array();
		$temp = $this->mdl_cbd_analisa->get_tree_item($idpro, $depth, $tgl_rab,$param)->result();
		if(count($temp))
		{			
			$i = 0;
			foreach($temp as $row){
		
				//$temp_harga = $this->mdl_cbd_analisa->get_tree_item_harga($idpro, $row->current_budget_item_tree)->row_array();
				$data[] = array(
					'current_budget_item_tree' => $row->current_budget_item_tree,
				...
				$q = sprintf("SELECT * FROM simpro_current_budget_item_tree WHERE id_proyek = '%d' AND trim(tree_parent_kode) = '$row->kode_tree' and tanggal_kendali = '$tgl_rab'",$row->id_proyek,$row->kode_tree);
				$query = $this->db->query($q);
				$is_child = $query->num_rows();

				if($is_child)
				{				
					// var_dump($row->current_budget_item_tree);
					$result[] = array_merge(
						$data[$i],
						array(
							'iconCls' => 'task-folder',
	...
	function gen_kode_tree($proyekid, $parentid, $kode_tree, $tgl_rab)
	{
		$sql_tree = sprintf("
					...
					ORDER BY current_budget_item_tree DESC", 
					$proyekid, 
					$parentid
					);
	...
	function del_tree_item($tgl_rab)
	{
		if($this->input->post('tree_item_id'))
		{
			...
			$qry2 = $this->db->query(sprintf("DELETE FROM simpro_current_budget_analisa_item_apek WHERE kode_tree = (select kode_tree from simpro_current_budget_item_tree WHERE current_budget_item_tree = '%d' and tanggal_kendali = '$tgl_rab') and tanggal_kendali = '$tgl_rab'", $this->input->post('tree_item_id')));
			$qry = $this->db->query(sprintf("DELETE FROM simpro_current_budget_item_tree WHERE current_budget_item_tree = '%d' and tanggal_kendali = '$tgl_rab'", $this->input->post('tree_item_id')));
	...
	{		
		...
		&& $this->input->post('id_proyek') && $this->input->post('current_budget_item_tree'))
		{
			$du = array(
				'current_budget_item_tree' => $this->input->post('current_budget_item_tree'),
			...
			$this->db->where('tanggal_kendali', $tgl_rab);
			$this->db->where('current_budget_item_tree', $this->input->post('current_budget_item_tree'));
			$this->db->where('id_proyek', $this->input->post('id_proyek'));
			$this->db->where('tree_parent_id', $this->input->post('tree_parent_id'));
			if($this->db->update('simpro_current_budget_item_tree', $du)) echo "Data berhasil diupdate";
				else echo "Data GAGAL diupdate!";
		}
	}
	...
	function get_parent_tree_id()
	{
		...
		$parentid = @$_SESSION['sess_current_budget_parent_id'];		
		return $this->session->userdata('current_budget_parent_tree_id') <> '' ? $this->session->userdata('current_budget_parent_tree_id') : $parentid;
	}

	/*
	function set_current_budget_item_tree()
	{
		if($this->input->post('id_proyek') && $this->input->post('current_budget_item_tree') && $this->input->post('kode_tree'))
		{
			$this->session->set_userdata('sess_id_proyek', $this->input->post('id_proyek'));
			$this->session->set_userdata('sess_current_budget_item_tree', $this->input->post('current_budget_item_tree'));
			$this->session->set_userdata('sess_kode_tree', $this->input->post('kode_tree'));
			$_SESSION['sess_id_proyek'] = $this->input->post('id_proyek');
			$_SESSION['sess_current_budget_item_tree'] = $this->input->post('current_budget_item_tree');
			$_SESSION['sess_kode_tree'] = $this->input->post('kode_tree');
		}
	}	
	
	function get_current_budget_item_tree()
	{
		if(isset($_SESSION['sess_id_proyek']) && isset($_SESSION['sess_current_budget_item_tree']))
		{
			$data = array(
				'id_proyek' => $_SESSION['sess_id_proyek'],
				'current_budget_item_tree' => $_SESSION['sess_current_budget_item_tree'],
	...
	*/
	...
	function del_analisa_satuan_item($tgl_rab)
	{
		if($this->input->post('tree_item_id'))
		{
			...
			// 	where
			// 	kode_analisa =
			// 	(SELECT kode_analisa FROM simpro_current_budget_analisa_item_apek
			// 	WHERE tanggal_kendali = '$tgl_rab' and kode_tree = '$ratid' and id_proyek = $proyek_id)";
			// $q_cek = $this->db->query($cek_data)->row();
			// if ($q_cek->count > 1) {
			// 	$sql_del_item_apek = "delete from simpro_current_budget_analisa_item_apek WHERE tanggal_kendali = '$tgl_rab' and kode_tree = '$ratid' and id_proyek = $proyek_id";
			// 	$q_del_item_apek = $this->db->query($sql_del_item_apek);
			// 	// $sql_upt_item_tree = "update simpro_current_budget_item_tree set kode_analisa = '' WHERE tanggal_kendali = '$tgl_rab' and kode_tree = '$ratid' and id_proyek = $proyek_id";
			
			// 	if($q_del_item_apek) echo "Data Analisa satuan berhasil dihapus.";
			// 	else echo "Data Analisa satuan GAGAL dihapus!";
			// } else {
			// 	// $sql_del_apek = "delete from simpro_current_budget_analisa_apek where parent_kode_analisa = 
			// 	// 	(SELECT kode_analisa FROM simpro_current_budget_analisa_item_apek
			...
			// 	// $sql_del_asat = "delete from simpro_current_budget_analisa_asat where kode_analisa = 
			// 	// 	(SELECT kode_analisa FROM simpro_current_budget_analisa_item_apek
			// 	// 	WHERE tanggal_kendali = '$tgl_rab' and kode_tree = '$ratid' and id_proyek = $proyek_id) and tanggal_kendali = '$tgl_rab' and id_proyek = $proyek_id";
			...
			// 	$q_del_item_apek = $this->db->query($sql_del_item_apek);
				// $sql_upt_item_tree = "";
			$qry2 = $this->db->query(sprintf("DELETE FROM simpro_current_budget_analisa_item_apek WHERE kode_tree = (select kode_tree from simpro_current_budget_item_tree WHERE current_budget_item_tree = '%d' and tanggal_kendali = '$tgl_rab' and id_proyek = $proyek_id) and tanggal_kendali = '$tgl_rab'", $this->input->post('tree_item_id')));
			...
			// $ratid = $this->input->post('tree_item_id');
			// $query = sprintf("
			// 	DELETE FROM simpro_current_budget_analisa_asat WHERE tanggal_kendali = '$tgl_rab' and kode_analisa = 
			// 	(
			// 		SELECT kode_analisa FROM simpro_current_budget_analisa_item_apek
			// 		WHERE tanggal_kendali = '$tgl_rab' and current_budget_item_tree = '%d'
			// 	);

			// 	DELETE FROM simpro_current_budget_analisa_apek where kode_analisa = 
			// 	(
			// 		SELECT kode_analisa FROM simpro_current_budget_analisa_item_apek
			// 		WHERE tanggal_kendali = '$tgl_rab' and current_budget_item_tree = '%d'
			// 	);			
				
			// 	DELETE FROM simpro_current_budget_analisa_item_apek WHERE tanggal_kendali = '$tgl_rab' and current_budget_item_tree = '%d';				
			// ", $ratid, $ratid, $ratid);		
	...
	public function get_daftar_analisa($id,$tgl_rab)
	{
		...
		$data = $this->mdl_cbd_analisa->get_daftar_analisa_pekerjaan($idproyek, $idkat,$tgl_rab);
		$this->_out($data);
	}
	...
	function tambah_daftar_analisa($tgl_rab)
	{
		...
		{
			$is_analisa_exists = $this->db->query(sprintf("SELECT * FROM simpro_current_budget_analisa_daftar WHERE kode_analisa ='%s' AND id_proyek = '%d' and tanggal_kendali = '$tgl_rab'", $this->input->post('kode_analisa'), $this->input->post('id_proyek')))->num_rows();
			...
			$qkode = sprintf("
				...
				FROM simpro_current_budget_analisa_daftar
			...
			if($is_analisa_exists)
			{
				...
				$this->db->update('simpro_current_budget_analisa_daftar', $data, $wdu);			
				echo "Data berhasil diupdate.";
			} else
			{				
				$this->db->insert('simpro_current_budget_analisa_daftar',$data);
	...
	{
		$idproyek = isset($idproyek) ? $idproyek : $this->get_idproyek();	
		$data = $this->mdl_cbd_analisa->get_harga_satuan_asat($idproyek,$tgl_rab);		
		$this->_out($data);	
	}
	...
	function update_harga_asat($tgl_rab)
	{
		if($this->input->post('id_proyek') && $this->input->post('kode_material'))
		{
			...
			$this->db->where('id_proyek', $this->input->post('id_proyek'));
			$this->db->where('tanggal_kendali', $tgl_rab);
			if($this->db->update('simpro_current_budget_analisa_asat', $postdata))
			{
				if($this->update_kode_current_budget($this->input->post('id_proyek'),$tgl_rab)) echo "Harga berhasil diupdate.";
					else echo "GAGAL update kode current_budget";
	...
	{				
		$q = sprintf("SELECT * FROM simpro_current_budget_item_tree WHERE tanggal_kendali = '$tgl_rab' and id_proyek = '%d' AND tree_parent_id = '%d'", $id_proyek, $id_tree);
		$query = $this->db->query($q);
		...
		{				
			...
			{
				$this->delete_tree_recursive($id_proyek, $v['current_budget_item_tree'], $tgl_rab);
			}
		} else
		{
			//current_budget_item_tree
			if($this->db->delete("simpro_current_budget_item_tree",array(
				"id_proyek"=>$id_proyek,
				"current_budget_item_tree"=>$id_tree,
				"tanggal_kendali"=>$tgl_rab
				))
			&& $this->db->delete("simpro_current_budget_analisa_item_apek",array(
				"id_proyek"=>$id_proyek,
	...
	function upload_daftar_analisa($tgl_rab)
	{
		...
		try {
			...
			} else
			{
				...
				{
					$qdel = sprintf("DELETE FROM simpro_current_budget_analisa_daftar WHERE id_proyek = '%d' and tanggal_kendali = '$tgl_rab'",$this->input->post('id_proyek'));
					$this->db->query($qdel);
					$sql_insert = sprintf("
						INSERT INTO simpro_current_budget_analisa_daftar(kode_analisa, nama_item, id_kat_analisa, id_satuan, id_proyek, tanggal_kendali)
						(
							SELECT 
							kode_analisa, 
							nama_item, 
	...
	function delete_analisa_pekerjaan($tgl_rab)
	{
		if($this->input->post('kode_analisa') && $this->input->post('id_proyek'))
		{
			...
			$del_daftar = $this->db->delete('simpro_current_budget_analisa_daftar', $where);
			$del_asat = $this->db->delete('simpro_current_budget_analisa_asat', $where);
			$del_apek = $this->db->delete('simpro_current_budget_analisa_apek', array('parent_kode_analisa'=>$this->input->post('kode_analisa'), 'id_proyek'=>$this->input->post('id_proyek'), 'tanggal_kendali'=>$tgl_rab));
	...
	function delete_asat_apek($tgl_rab)
	{
		if($this->input->post('kode_analisa') && $this->input->post('id_proyek'))
		{
			...
			$del_asat = $this->db->delete('simpro_current_budget_analisa_asat', array(
				'kode_analisa'=>$this->input->post('kode_analisa'), 
				'id_proyek'=>$this->input->post('id_proyek'), 
				'tanggal_kendali'=>$tgl_rab
				));
			$del_apek = $this->db->delete('simpro_current_budget_analisa_apek', array(
	...
	{
		$data = $this->mdl_cbd_analisa->get_data_analisa_pekerjaan($idtender,$tgl_rab);
		$this->_out($data);	
	}
	...
	function edit_koefisien_apek()
	{
		if($this->input->post('id_analisa_apek'))
		{
			...
			if($this->db->update('simpro_current_budget_analisa_apek', array('koefisien'=>$this->input->post('koefisien'))))
				echo "Data berhasil diupdate.";
					else echo "data GAGAL diupdate!";
		}
	}	
	...
	function get_data_ansat()
	{
		...
		{
			$data = $this->mdl_cbd_analisa->get_ansat($this->input->get('subbidang_kode'));
		} else $data = $this->mdl_cbd_analisa->get_ansat('');
		$this->_out($data);	
	}
		...
		&& $this->input->post('koefisien') && $this->input->post('id_proyek'))
		{
			...
			foreach($km as $k=>$v)
			{
				$cek_data = $this->db->query("SELECT simpro_tbl_detail_material.detail_material_nama 
					...
					on simpro_current_budget_analisa_asat.kode_material = simpro_tbl_detail_material.detail_material_kode 
					where simpro_current_budget_analisa_asat.kode_material = '$v' 
					and simpro_current_budget_analisa_asat.id_proyek = $var_analisa[id_proyek] 
					and simpro_current_budget_analisa_asat.id_data_analisa = $var_analisa[id_data_analisa]");
				...
				if ($cek_data->result()) {
					...
					$this->db->insert('simpro_current_budget_analisa_asat',$data);
				}						
				$i++;
			}
			...
			// if($this->db->insert_batch('simpro_current_budget_analisa_asat',$data)) 
			// {
				if($this->update_kode_current_budget($this->input->post('id_proyek'),$tgl_rab)) echo $m;
					else echo "GAGAL update kode RAP";
			// } else echo "Data GAGAL disimpan!";
		}
	}
	
	function update_kode_current_budget($id,$tgl_rab)
	{
		$error = FALSE;
		$sql_kode = "
		...
		foreach($kcurrent_budget as $k=>$v)
		{
			// $sql = sprintf("
			// 	SELECT 
			// 		DISTINCT(simpro_current_budget_analisa_asat.kode_material) as kode_material,				
			// 		LEFT(simpro_current_budget_analisa_asat.kode_material,3),
			// 		simpro_current_budget_analisa_asat.id_analisa_asat,
			// 		simpro_rat_analisa_kategori.kode_kat,
			// 		COALESCE(simpro_current_budget_analisa_asat.harga, 0) AS harga
			// 	FROM simpro_current_budget_analisa_asat
			// 	LEFT JOIN simpro_rat_analisa_kategori ON simpro_rat_analisa_kategori.subbidang_kode = LEFT(simpro_current_budget_analisa_asat.kode_material,3)
			...
			// 	GROUP BY simpro_current_budget_analisa_asat.kode_material, 
			// 		simpro_rat_analisa_kategori.kode_kat,
			// 		simpro_current_budget_analisa_asat.harga,
			// 		simpro_current_budget_analisa_asat.id_analisa_asat				
			// 	ORDER BY simpro_current_budget_analisa_asat.kode_material ASC", $id, $v['kode_kat']);
	...
	function tambah_apek($tgl_rab)
	{
		if($this->input->post('kode_analisa') && $this->input->post('id_data_analisa') && $this->input->post('id_proyek'))
		{
			...
			{		
				...
				if ($this->db->get_where('simpro_current_budget_analisa_apek',$var_w)->num_rows() > 0) {
			...
			if(strtoupper($var_analisa['kode_analisa']) != '')
			{
				if(!$error_duplicate)
				{
					...
					{
						if ($data != "") {
							if($this->db->insert_batch('simpro_current_budget_analisa_apek',$data)) echo "Data telah disimpan.";
								else echo "Data GAGAL disimpan!";
						}
	...
	function tambah_apek_current_budget($tgl_rab)
	{
		if($this->input->post('kode_analisa') && $this->input->post('id_data_analisa') && $this->input->post('id_proyek'))
		{
			$is_data = 0;
			$var_item = $this->get_current_budget_item_tree();			
			# cek data
			$q_cek = sprintf("SELECT * FROM simpro_current_budget_analisa_item_apek WHERE current_budget_item_tree='%d' AND id_proyek='%d'
								and tanggal_kendali = '$tgl_rab'",
							$var_item['current_budget_item_tree'], $this->input->post('id_proyek')
							);						
			...
			$data = array(
				...
				'current_budget_item_tree' => $var_item['current_budget_item_tree'],
			...
			{			
				$this->db->where('id_proyek', $this->input->post('id_proyek'));
				$this->db->where('current_budget_item_tree', $var_item['current_budget_item_tree']);
				if($this->db->update('simpro_current_budget_analisa_item_apek', $data)) echo "Data telah diupdate.";
					else echo "Data GAGAL diupdate!";
					
			} else 
			{
				if($this->db->insert('simpro_current_budget_analisa_item_apek',$data)) echo "Data telah disimpan.";
					else echo "Data GAGAL disimpan!";
			}			
		}
	}
	...
	{
		$q_cek_level = sprintf("SELECT * FROM simpro_current_budget_analisa_asat WHERE kode_analisa = '%s'", $kd_analisa);
	...
	public function delete_apek()
	{
		if($this->input->post('id_analisa_apek'))
		{
			if($this->db->delete('simpro_current_budget_analisa_apek', array('id_analisa_apek'=>$this->input->post('id_analisa_apek'))))
				echo "Data berhasil dihapus.";
					else echo "Data GAGAL dihapus!";
		}
	}
	...
	function delete_analisa($tgl_rab)
	{
		if($this->input->post('kode_analisa') && $this->input->post('id_data_analisa') && $this->input->post('id_proyek'))
		{
			...
			foreach($kd_analisa as $k=>$v)
			{
				...
				$id_item_apek = $this->db->delete('simpro_current_budget_analisa_item_apek',$arr_w);

				$where = array('kode_analisa'=>$v, 'id_proyek'=>$idtender,'tanggal_kendali'=>$tgl_rab);
				$del_daftar = $this->db->delete('simpro_current_budget_analisa_daftar', $where);
				$del_asat = $this->db->delete('simpro_current_budget_analisa_asat', $where);
				$del_asat = $this->db->delete('simpro_current_budget_analisa_apek', $where);
				$del_apek = $this->db->delete('simpro_current_budget_analisa_apek', array('parent_kode_analisa'=>$v, 'id_proyek'=>$idtender,'tanggal_kendali'=>$tgl_rab));
				if($del_daftar && $del_asat && $del_apek)
				{
					$success = TRUE;
				} else $success = FALSE;
	...
	function delete_ansat($tgl_rab)
	{
		if($this->input->post('id_analisa_asat'))
		{
			...
			foreach($idasat as $k=>$v)
			{
				...
				{
					if($this->db->delete('simpro_current_budget_analisa_apek',array('id_analisa_apek'=>$v,'tanggal_kendali'=>$tgl_rab)))
						$success = TRUE;
							else $success = FALSE;
				} else
				{
					if($this->db->delete('simpro_current_budget_analisa_asat',array('id_analisa_asat'=>$v,'tanggal_kendali'=>$tgl_rab)))
						$success = TRUE;
							else $success = FALSE;
				}
				$i++;
	...
	function total_current_budgeta($tgl_rab)
	{
		$data = $this->mdl_cbd_analisa->total_current_budgeta($this->session->userdata('proyek_id'),$tgl_rab);
		if(isset($data['data']['total_current_budgeta'])) return $data['data']['total_current_budgeta'];
	}
	
	function current_budgeta_to_xls($tgl_rab="")
	{
		$this->load->library('excel');
		$dp = $this->get_data_proyek_byid(); //
		$current_budgetas = $this->mdl_cbd_analisa->current_budget_current_budgeta($this->session->userdata('proyek_id'),$tgl_rab);		
		$current_budgeta = $current_budgetas['data'];
		$total_current_budgeta = $this->total_current_budgeta($tgl_rab);
		$persen_current_budgeta = 0; //$this->persen_rata($idtender);
		$data = array(
			'data_proyek' => $dp['data']
		);	
		...
		$this->excel->getActiveSheet()->setTitle('current_budgeta');
		...
		if(count($current_budgeta) > 0)
		{
			...
			$nextsub = "";
			$start_A = 12;
			for($a=0; $a < count($current_budgeta); $a++)
			{
				$sub = $current_budgeta[$a]['simpro_tbl_subbidang'];
				if($sub <> $subbidang)
				{				
					$this->excel->getActiveSheet()->setCellValue(sprintf('A%d', $start_A),$current_budgeta[$a]['simpro_tbl_subbidang']);
					$this->excel->getActiveSheet()->mergeCells(sprintf('A%d:H%d', $start_A, $start_A));
					$subbidang = $sub;
					$subtotal = $current_budgeta[$a]['subtotal'];
					$start_A++;					
				} else 
				{
					$subtotal = $subtotal + $current_budgeta[$a]['subtotal'];
				}
							
				$this->excel->getActiveSheet()->setCellValue(sprintf('A%d', $start_A), $i);
				$this->excel->getActiveSheet()->setCellValue(sprintf('B%d', $start_A), $current_budgeta[$a]['kode_rap']);
				$this->excel->getActiveSheet()->setCellValue(sprintf('C%d', $start_A), $current_budgeta[$a]['kd_material']);
				$this->excel->getActiveSheet()->setCellValue(sprintf('D%d', $start_A), $current_budgeta[$a]['detail_material_nama']);
				$this->excel->getActiveSheet()->setCellValue(sprintf('E%d', $start_A), $current_budgeta[$a]['detail_material_satuan']);
				$this->excel->getActiveSheet()->setCellValue(sprintf('F%d', $start_A), number_format($current_budgeta[$a]['total_volume'],2));
				$this->excel->getActiveSheet()->setCellValue(sprintf('G%d', $start_A), number_format($current_budgeta[$a]['harga'],2));
				$this->excel->getActiveSheet()->setCellValue(sprintf('H%d', $start_A), number_format($current_budgeta[$a]['subtotal'],2));
				
				$nextsub = isset($current_budgeta[$a+1]['simpro_tbl_subbidang']) ? $current_budgeta[$a+1]['simpro_tbl_subbidang'] : '';
				if($nextsub <> $sub)
				{
					$start_A++;				
			...
			$this->excel->getActiveSheet()->setCellValue(sprintf('A%d', $lasta),'TOTAL current_budget(A)');
			$this->excel->getActiveSheet()->setCellValue(sprintf('H%d', $lasta),$total_current_budgeta);
			...
			$this->excel->getActiveSheet()->setCellValue(sprintf('H%d', $lasta+1),$persen_current_budgeta);
		...
		# output
		$filename = sprintf('current_budgeta-%s.xlsx', trim($nama_proyek)); 		
		...
		if ($page=="") {
			...
			switch ($page) {
				case 'current_budget':					
					...
					$q_current_budgeta = $this->get_data_print($idpro,$tgl_rab);

					$x = 5;

					$tot = 0;
					
					if ($q_current_budgeta) {
						foreach ($q_current_budgeta as $row) {
							$kode_tree = $row->kode_tree;
							$kode_analisa = $row->kode_analisa;
