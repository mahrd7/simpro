...
Ext.require([
    ...
    var store_tgl = Ext.create('Ext.data.Store', {
        ...
        autoLoad: false,        
         proxy: {
             type: 'ajax',
             url: '<?php echo base_url() ?>pengendalian/get_tgl/rencana_kontrak',
             reader: {
                 type: 'json',
                 root: 'data'
             }
...
var grid_lpf = Ext.create('Ext.grid.Panel', {
        ...
        store: store_tgl,
        columns: [
            {text: "BULAN Cut Off", flex:1, sortable: true, dataIndex: 'blnnama'},
            {text: "TAHUN Cut Off", flex:1, sortable: true, dataIndex: 'thn'},
            ...
            handler: function(grid,rowIndex,colIndex){
                ...
                Ext.MessageBox.confirm('Proses Pelaporan', 'Apakah anda akan melakukan proses pelaporan?',function(resbtn){
                    if(resbtn == 'yes')
                    {
                        Ext.Ajax.request({
                            url: '<?=base_url();?>pengendalian/get_data_cut_off/'+tgl_rab,
                            success: function() {
                            Ext.Msg.alert( "Status", "Proses Pelaporan Sukses..!");   
                            },
                            failure: function() {
