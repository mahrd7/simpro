...
<script type="text/javascript">
    ...
    Ext.onReady(function() {
		...
		var storeANSAT = Ext.create('Ext.data.Store', {
			 ...
			 proxy: {
				type: 'ajax',
				url: '<?=base_url();?>pengendalian/ctg_analisa/get_data_ansat/<?=$id_proyek;?>/<?=$tgl_rab;?>',
				reader: {
					type: 'json',
					root: 'data',
					totalProperty: 'total'
					...
					items: [
						{
							...
							handler: function(){          
									...
									{
										Ext.Ajax.request({
											url: '<?=base_url();?>pengendalian/ctg_analisa/tambah_apek/<?=$tgl_rab;?>',
											method: 'POST',											
											params: {																	
					...
					items: [
						{
							...
							handler: function(){
								...
								{
									Ext.Ajax.request({
										url: '<?=base_url();?>pengendalian/ctg_analisa/tambah_ansat/<?=$tgl_rab;?>',
										method: 'POST',
										params: {			
											/*
		...
		var winANSAT = Ext.widget('window', {
								...
								handler: function(){        
									...
									if(analisa != '')
									{
										...
										function(resbtn){
												...
												{
												Ext.Ajax.request({
													url: '<?=base_url();?>pengendalian/ctg_analisa/delete_ansat/<?php echo $tgl_rab ?>',
													method: 'POST',											
													params: {
			...
			var storeAnalisaPekerjaan = Ext.create('Ext.data.Store', {
				...
				proxy: {
					 type: 'ajax',
					 url: '<?php echo base_url() ?>pengendalian/ctg_analisa/get_daftar_analisa/<?=$id_proyek;?>/<?=$tgl_rab;?>',
					 reader: {
						 type: 'json',
						 root: 'data'
					 }
			...
			var APcellEditing = Ext.create('Ext.grid.plugin.RowEditing', {
					...
					listeners : {
						'edit' : function() {						
							...
							Ext.Ajax.request({
								url: '<?=base_url();?>pengendalian/ctg_analisa/tambah_daftar_analisa/<?=$tgl_rab;?>',
								method: 'POST',
								params: {
									'id_proyek' : <?=$id_proyek;?>,
			...
			var storeEditHargaASAT = Ext.create('Ext.data.Store', {
				...
				proxy: {
					 type: 'ajax',
					 url: '<?php echo base_url() ?>pengendalian/ctg_analisa/edit_harga_satuan_asat/<?=$id_proyek;?>/<?=$tgl_rab;?>',
					 reader: {
						 type: 'json',
						 root: 'data'
					 }
				...
				plugins: Ext.create('Ext.grid.plugin.RowEditing', {
					...
					listeners : {
						edit : function() {
							...
							Ext.Ajax.request({
								url: '<?=base_url();?>pengendalian/ctg_analisa/update_harga_asat/<?=$tgl_rab;?>',
								method: 'POST',
								params: {
									'id_proyek' : <?=$id_proyek;?>,
        ...
        var frmUploadAnalisa = Ext.widget({
			xtype: 'form',
			layout: 'form',
			url: '<?php echo base_url(); ?>pengendalian/ctg_analisa/upload_daftar_analisa/<?=$tgl_rab;?>',
			frame: false,
			bodyPadding: '5 5 0',
			width: 350,
			fieldDefaults: {
					...
					{text: "",xtype: 'actioncolumn', flex:1,  align: 'center', sortable: false,icon:'<?=base_url();?>assets/images/application_go.png',
						handler: function(grid, rowIndex, colIndex){        
							var rec = storeAnalisaPekerjaan.getAt(rowIndex);
								Ext.Ajax.request({
									url: '<?=base_url();?>pengendalian/ctg_analisa/set_analisa_itemid',
									method: 'POST',
									params: {
					...
					{text: "",xtype: 'actioncolumn', flex:1, align: 'center', tooltip: 'Delete item ini', sortable: true,icon:'<?=base_url();?>assets/images/delete.gif',
						handler: function(grid, rowIndex, colIndex){        
							...
							Ext.MessageBox.confirm('Delete item', 'Apakah anda akan menghapus item ('+item+') ini?',function(resbtn){
									...
									{
										Ext.Ajax.request({
											url: '<?=base_url();?>pengendalian/ctg_analisa/delete_analisa_pekerjaan/<?=$tgl_rab;?>',
											method: 'POST',
											params: {
												'kode_analisa':id,
												'id_proyek':<?=$id_proyek;?>,
					...
					{text: "",xtype: 'actioncolumn', flex:1, align: 'center', tooltip: 'Hapus seluruh analisa untuk item ini', sortable: true,icon:'<?=base_url();?>assets/images/cross.gif',
						handler: function(grid, rowIndex, colIndex){        
							...
							Ext.MessageBox.confirm('Delete item', 'Apakah anda akan menghapus seluruh Analisa Satuan Pekerjaan untuk item ('+item+') ini?',function(resbtn){
									...
									{
										Ext.Ajax.request({
											url: '<?=base_url();?>pengendalian/ctg_analisa/delete_asat_apek/<?=$tgl_rab;?>',
											method: 'POST',
											params: {
												'kode_analisa':id,
												'id_proyek':<?=$id_proyek;?>,
						...
						items: [
							{
								...
								iconCls: 'icon-back',
								handler: function(){    
									window.location = '<?=base_url();?>pengendalian/cost_to_go/<?=$kunci;?>/<?=$tgl_rab;?>';
								}
							},'-',
							{
								text:'Tambah Data',
							...
							},'-',
							{
								...
								handler: function(){          
									Ext.Ajax.request({
										url: '<?=base_url();?>pengendalian/ctg_analisa/<?=$tgl_rab;?>',
										method: 'POST',
										params: {
											'clearsearch' : 1,
											'page' : 1,
								...
								handler: function(){          
									...
									if(analisa != '')
									{
										...
										function(resbtn){
												...
												{
												Ext.Ajax.request({
													url: '<?=base_url();?>pengendalian/ctg_analisa/delete_analisa/<?php echo $tgl_rab ?>',
													method: 'POST',											
													params: {												
														'kode_analisa' : analisa.join(','),
