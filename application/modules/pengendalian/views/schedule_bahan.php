...
Ext.require([
    ...
    var storemutu = Ext.create('Ext.data.Store', {
        ...
        proxy: {
         type: 'ajax',
         url: '<?php echo base_url() ?>pengendalian/getsch/bahan',
         reader: {
             type: 'json',
             root: 'data'
         }
    ...
    storeparent = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: '<?php echo base_url(); ?>pengendalian/getschdetail/bahan',
            reader: {
                type: 'json',
                root: 'data'
            }
...
Ext.onReady(function() {
    ...
    var urls = '<?php echo base_url(); ?>pengendalian/getsch/bahan';
        ...
        columns: [
            ...
            var tahap_kendali_id = rec.get('tahap_kendali_id');
                ...
                Ext.MessageBox.confirm('Delete item', 'Apakah anda akan menghapus item ini?',function(resbtn){
                    ...
                    {
                        Ext.Ajax.request({
                             url: '<?php echo base_url(); ?>pengendalian/deletesch/bahan',
                                method: 'POST',
                                params: {
                                    'proyek_id' :  proyek_id,
    ...
    var viewport = Ext.create('Ext.Viewport', {
        ...
        items: [{
            ...
            region: 'west',
            layout: 'fit',
            title: 'Gantt Schedule Bahan',
            width: '100%',
            height: '50%',
            border: 0,
            html: '<iframe src="<?php echo base_url() ?>pengendalian/schedule_cart/bahan" border=0 width="100%" height="100%" id="gantframe"></iframe>'
        }]
    });

function frmaddfirst(){
    var urls = '<?php echo base_url(); ?>pengendalian/insertschproyek/bahan';
                ...
                if (formdoc.getForm().isValid()){
                    formdoc.getForm().submit({
                        success: function(fp, o) {                          
                            ...
                            Ext.MessageBox.alert('Schedule Bahan','Insert successfully..!');
                            document.getElementById('gantframe').contentWindow.location.reload();
                        },
        ...
        columns: [
            ...
            handler: function(grid, rowIndex, colIndex){
                ...
                Ext.MessageBox.confirm('Delete item', 'Apakah anda akan menghapus item ini?',function(resbtn){
                    ...
                    {
                        Ext.Ajax.request({
                             url: '<?=base_url();?>pengendalian/deleteschparent/bahan',
                                method: 'POST',
                                params: {
                                    'id' :  idparent
                                },                              
...
function frma(id){
    var urls = '<?php echo base_url(); ?>pengendalian/insertdetailsch/bahan';
    ...
    grid = Ext.create('Ext.grid.plugin.RowEditing',{
        ...
        listeners:{
            afteredit: function(rec,obj) {
                ...
                Ext.Ajax.request({
                     url: '<?=base_url();?>pengendalian/update_bobot_sch_parent/bahan',
                        method: 'POST',
                        params: {
                            'id' :  id,
