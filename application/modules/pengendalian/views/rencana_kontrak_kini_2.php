...
var kode_induk;
    Ext.define('mdl', {
        ...
        fields: [
            ...
            {name: 'tahap_total_kendali_new', mapping: 'tahap_total_kendali_new', type:'float'},
            {name: 'tahap_harga_satuan_kendali_new', mapping: 'tahap_harga_satuan_kendali_new'},            
            {name: 'tahap_volume_kendali_kurang', mapping: 'tahap_volume_kendali_kurang', type:'float'},
            ...
            {name: 'rencana_harga_satuan_eskalasi', mapping: 'rencana_harga_satuan_eskalasi', type:'float'},            
            {name: 'is_nilai', mapping: 'is_nilai'},
            {name: 'tahap_total_kendali_kurang', mapping: 'tahap_total_kendali_kurang', type:'float'},
            {name: 'total_tambah_kurang', mapping: 'total_tambah_kurang', type:'float'},
            {name: 'total_volume_rencana', mapping: 'total_volume_rencana', type:'float'},            
            {name: 'tot_rencana1', mapping: 'tot_rencana1', type:'float'},
            {name: 'tot_rencana2', mapping: 'tot_rencana2', type:'float'},
            {name: 'vol_tambah_kurang', mapping: 'vol_tambah_kurang'},
            {name: 'proyek_id', mapping: 'proyek_id'},            
            {name: 'user_update', mapping: 'user_update'},
            {name: 'id_rencana_kontrak_terkini', mapping: 'id_rencana_kontrak_terkini'}
         ]
    });
    ...
    var storesatuan = Ext.create('Ext.data.Store', {
        ...
        proxy: {
         type: 'ajax',
         url: '<?php echo base_url() ?>pengendalian/getlistsatuan/rkk',
         reader: {
             type: 'json',
             root: 'data'
         }
    ...
    var store_get_sub_kode = Ext.create('Ext.data.Store', {
        ...
        proxy: {
         type: 'ajax',
         url: '<?php echo base_url() ?>pengendalian/get_sub_kode/rkk',
         reader: {
             type: 'json',
             root: 'data'
         }
     ...
     var store_get_kode = Ext.create('Ext.data.Store', {
        ...
        proxy: {
         type: 'ajax',
         url: '<?php echo base_url() ?>pengendalian/get_kode/rkk',
         reader: {
             type: 'json',
             root: 'data'
         }
    ...
    var storeterkini = Ext.create('Ext.data.TreeStore', {
        ...
        autoLoad: false,        
             proxy: {
                ...
                async: false,
                type: 'ajax',
                url: '<?php echo base_url() ?>pengendalian/get_data_kk/rkk',
                reader: 'json'
             },
        listeners:{
            beforeload:function(){
    ...
    var storesubkontrakterkini = Ext.create('Ext.data.Store', {
        ...
        proxy: {
         type: 'ajax',
         url: '<?php echo base_url() ?>pengendalian/get_sub_kontrk_terkini/rkk',
         reader: {
             type: 'json',
             root: 'data'
         }
...
Ext.onReady(function() {
    ...
    var cellEditing = Ext.create('Ext.grid.plugin.RowEditing', {
        ...
        listeners: {
            ...
            afteredit: function(rec,obj) {
                ...
                id = data.id_rencana_kontrak_terkini;
                ...
                Ext.Ajax.request({
                     url: '<?=base_url();?>pengendalian/update_kk/rkk',
                        method: 'POST',
                        params: {
                            'id' :  id,
    ...
    var grid = Ext.create('Ext.tree.Panel', {
        ...
        height: '100%',
        autoScroll:true,
        title: 'ADDENDUM/AMANDEMEN KONTRAK S/D <?php echo $bln1; ?> Tahun <?php echo $thn; ?>',
        columns: [
        {text: "",xtype: 'actioncolumn', width:25,  sortable: true,icon:'<?=base_url();?>assets/images/delete.gif',
            ...
            handler: function(grid, rowIndex, colIndex, actionItem, event, rec, row){
                // rec = storeterkini.getAt(rowIndex);
                id = rec.get('id_rencana_kontrak_terkini');
                rab_kode = rec.get('rab_tahap_kode_kendali');
                kode = rec.get('tahap_kode_kendali');
                if (rab_kode == kode) {
                    Ext.MessageBox.confirm('Reset', 'Apakah anda akan mereset data item ini?',function(resbtn){
                        if(resbtn == 'yes')
                            {
                                Ext.Ajax.request({
                                url: '<?=base_url();?>pengendalian/reset/item_rencana_kontrak_terkini',
                                method: 'POST',
                                params: {
                                    'id':id
                                },                              
                    ...
                    Ext.MessageBox.confirm('Delete', 'Apakah anda akan menghapus data item ini?',function(resbtn){
                        if(resbtn == 'yes')
                            {
                                Ext.Ajax.request({
                                url: '<?=base_url();?>pengendalian/delete_data/item_rencana_kontrak_terkini',
                                method: 'POST',
                                params: {
                                    'id':id,
                                    'kode':kode,
        ...
        {text: "",xtype: 'actioncolumn', width:25,  sortable: true,icon:'<?=base_url();?>assets/images/add.gif',
            ...
            {text: "RENCANA KONTRAK TERKINI", sortable: false,
            columns:[
...
function tambah(kode){
    ...
    var frmadd = Ext.create('Ext.form.Panel', {     
        url: '<?php echo base_url() ?>pengendalian/insert/rencana_kontrak_terkini',
        id:'frmadd ',
...
function link(id,kode){
    ...
    var winadd = Ext.create('Ext.Window', {
        title: 'FORM SUB RENCANA KONTRAK TERKINI',
        closeAction: 'hide',
        width: '80%',
        height: '80%',
        layout: 'fit',
...
function tambahlink(kode,kode_induk){
    var frmadd = Ext.create('Ext.form.Panel', {     
        url: '<?php echo base_url() ?>pengendalian/insert/sub_rencana_kontrak_terkini/rkk',
        width:'100%',
        height:'100%',
    ...
    var winadd = Ext.create('Ext.Window', {
        title: 'FORM TAMBAH SUB RENCANA KONTRAK TERKINI',
        closeAction: 'hide',
        width: 500,
        height: '80%',
        layout: 'fit',
