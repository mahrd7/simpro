...
class Mdl_ctg_Analisa extends CI_Model {
	...
	function get_tree_item($idpro, $tree_parent=0, $tgl_rab,$param)
	{
		...
		$query =
			...
			(select count(kode_tree) 
				from (select * from simpro_costogo_item_tree 
					where lower(tree_item) like lower('%".$param."%') 
					and id_proyek = $idpro 
					and costogo_item_tree = x.costogo_item_tree
			...
			from simpro_costogo_item_tree a
			...
			LEFT JOIN simpro_costogo_analisa_item_apek ON simpro_costogo_analisa_item_apek.kode_tree = simpro_costogo_item_tree.kode_tree and simpro_costogo_analisa_item_apek.id_proyek = $idpro
			LEFT JOIN (
				SELECT 
			...
			WHERE simpro_costogo_item_tree.id_proyek = $idpro and simpro_costogo_item_tree.tanggal_kendali = '$tgl_rab' 
	...
	{
		$sql = "update simpro_costogo_analisa_asat set
			harga = a.harga
			from (
			select * from simpro_costogo_analisa_asat where id_proyek = $idproyek and tanggal_kendali = '$tgl_rab' and harga != 0
			) a
			where simpro_costogo_analisa_asat.id_proyek = $idproyek and simpro_costogo_analisa_asat.tanggal_kendali = '$tgl_rab' and simpro_costogo_analisa_asat.harga = 0 and simpro_costogo_analisa_asat.kode_material = a.kode_material";
		$this->db->query($sql);
	}
	...
	{	
		...
		$query = sprintf("
				...
				(
					SELECT 
						simpro_costogo_analisa_apek.tanggal_kendali,
					...
					WHERE simpro_costogo_analisa_apek.id_proyek= '%d'
					and simpro_costogo_analisa_apek.tanggal_kendali = '$tgl_rab'
					ORDER BY 
						simpro_costogo_analisa_apek.parent_kode_analisa,				
						simpro_costogo_analisa_apek.kode_analisa
					ASC					
		...
		$query = sprintf(
			...
			SELECT 
				(
					...
					FROM 
						simpro_costogo_analisa
					WHERE 
						id_proyek = '%d' AND 
						costogo_item_tree = '%d'
					GROUP BY 
						id_proyek, costogo_item_tree						
				) AS harga,				
				(
					SELECT
						SUM(aharga * akoefisien) as subtotal
	...
	function rat_rata($idproyek,$tgl_rab)
	{
		...
		// 				WHERE simpro_costogo_analisa_apek.id_proyek= '%d' and simpro_costogo_analisa_apek.tanggal_kendali = '$tgl_rab'
		// 				GROUP BY  
				...
				FROM (				
					...
					(				
						...
						(
							...
							WHERE simpro_costogo_analisa_apek.id_proyek = $idproyek and tanggal_kendali = '$tgl_rab'
							GROUP BY  
							tbl_asat.kode_material,
							tbl_asat.koefisien,
							simpro_costogo_analisa_apek.kode_analisa,
					...
					INNER JOIN simpro_costogo_analisa_item_apek ON simpro_costogo_analisa_item_apek.kode_analisa = tbl_asat_apek.parent_kode_analisa AND simpro_costogo_analisa_item_apek.id_proyek = $idproyek and tanggal_kendali = '$tgl_rab'
	...
	{				
		if($idkat)
		{
			$q_idkat = sprintf("AND simpro_costogo_analisa_daftar.id_kat_analisa = '%d'", $idkat);
		} else $q_idkat = "";
		$query = "
			SELECT 
			...
			simpro_costogo_analisa_daftar
			LEFT JOIN simpro_tbl_satuan ON simpro_tbl_satuan.satuan_id = simpro_costogo_analisa_daftar.id_satuan
			LEFT JOIN simpro_rat_analisa_kategori ON simpro_rat_analisa_kategori.id_kat_analisa = simpro_costogo_analisa_daftar.id_kat_analisa			
			LEFT JOIN (
				SELECT 
					DISTINCT ON(kode_analisa)
					kode_analisa,
				...
				and tanggal_kendali = '$tgl_rab'
				GROUP BY kode_analisa			
			) as tbl_harga ON tbl_harga.kode_analisa = simpro_costogo_analisa_daftar.kode_analisa		
			LEFT JOIN 
			(
				SELECT 
					DISTINCT ON(simpro_costogo_analisa_apek.parent_kode_analisa)
					simpro_costogo_analisa_apek.parent_kode_analisa,
					SUM(simpro_costogo_analisa_apek.koefisien * tbl_harga_asat.harga) as harga,
					coalesce((count('a')),0) as c
				FROM simpro_costogo_analisa_apek
				INNER JOIN (
					SELECT 
					...
					and tanggal_kendali = '$tgl_rab'
					GROUP BY kode_analisa							
				) as tbl_harga_asat ON tbl_harga_asat.kode_analisa = simpro_costogo_analisa_apek.kode_analisa
				WHERE simpro_costogo_analisa_apek.id_proyek= '".$idproyek."' and simpro_costogo_analisa_apek.tanggal_kendali = '$tgl_rab'
				GROUP BY simpro_costogo_analisa_apek.parent_kode_analisa			
			) as tbl_harga_apek ON tbl_harga_apek.parent_kode_analisa = simpro_costogo_analisa_daftar.kode_analisa			
			WHERE 
				simpro_costogo_analisa_daftar.id_proyek= '".$idproyek."'
				and simpro_costogo_analisa_daftar.tanggal_kendali = '$tgl_rab'
				".$q_idkat."
		";
		...
		{
			$search = $_REQUEST['query'];
			$q_search = "AND (simpro_costogo_analisa_daftar.nama_item @@ to_tsquery('".addslashes($search).":*') OR simpro_costogo_analisa_daftar.kode_analisa @@ to_tsquery('".addslashes($search).":*'))";
		} else
		{
			$search = "";
			$q_search = "";			
		}		
		$qadd = "ORDER BY simpro_costogo_analisa_daftar.kode_analisa ASC";
		$query .= $q_search . $qadd;
	...
	function get_harga_satuan_asat($idtender,$tgl_rab)
	{
		$query = sprintf("
			SELECT 
				simpro_costogo_analisa_asat.kode_material,
				simpro_tbl_detail_material.detail_material_nama,
				simpro_tbl_detail_material.detail_material_satuan,
				simpro_tbl_detail_material.detail_material_nama,
				avg(simpro_costogo_analisa_asat.harga) as harga,
			...
			LEFT JOIN simpro_tbl_detail_material ON simpro_tbl_detail_material.detail_material_kode = simpro_costogo_analisa_asat.kode_material
			INNER JOIN simpro_tbl_subbidang ON simpro_tbl_subbidang.subbidang_kode = LEFT(simpro_tbl_detail_material.detail_material_kode,3)
			WHERE simpro_costogo_analisa_asat.id_proyek = '%d' and simpro_costogo_analisa_asat.tanggal_kendali = '$tgl_rab'
			group by
				simpro_costogo_analisa_asat.kode_material,
				simpro_tbl_detail_material.detail_material_nama,
				simpro_tbl_detail_material.detail_material_satuan,
		...
		", $idtender);
		$data = $this->reqcari($query, array('simpro_costogo_analisa_asat.kode_material', 'simpro_tbl_detail_material.detail_material_nama'));
		if(count($data))
		{
			return $data;
		} else return false;
	...
	{	
		$query = sprintf("
			SELECT 
				simpro_costogo_analisa_apek.*,
				(simpro_costogo_analisa_apek.parent_kode_analisa || ' - ' || simpro_costogo_analisa_daftar.nama_item || ' (' || simpro_tbl_satuan.satuan_nama || ') ') AS apek_kat, 				
				ad.nama_item AS nama_item,
				simpro_costogo_analisa_daftar.nama_item AS parent_item,
				simpro_costogo_analisa_daftar.id_satuan,
				simpro_tbl_satuan.satuan_nama,
				COALESCE(tbl_harga.harga,0) AS harga,
				COALESCE(tbl_harga.harga * koefisien, 0) AS subtotal
		...
		if(isset($_REQUEST['query']))
		{
			$search = $_REQUEST['query'];
			$q_search = "AND (
				simpro_costogo_analisa_daftar.nama_item @@ to_tsquery('".addslashes($search).":*') 
				OR simpro_costogo_analisa_apek.kode_analisa @@ to_tsquery('".addslashes($search).":*')
				OR simpro_costogo_analisa_apek.parent_kode_analisa @@ to_tsquery('".addslashes($search).":*')
	...
	{				
		$query = "
			...
			(
				...
				WHERE simpro_costogo_analisa_apek.id_proyek = '".$idtender."'
				and simpro_costogo_analisa_apek.tanggal_kendali = '$tgl_rab'
				GROUP BY simpro_costogo_analisa_apek.parent_kode_analisa			
			) as tbl_harga_apek ON tbl_harga_apek.parent_kode_analisa = simpro_costogo_analisa_daftar.kode_analisa
			WHERE 
				simpro_costogo_analisa_daftar.id_proyek = '".$idtender."'
				AND simpro_costogo_analisa_daftar.id_kat_analisa = '10'
				and simpro_costogo_analisa_daftar.tanggal_kendali = '$tgl_rab'
		";
	...
	function total_costogoa($idproyek,$tgl_rab)
	{
		$sql = sprintf("
			SELECT ROUND(SUM(subtotal)) AS total_costogoa FROM (
				SELECT 
					DISTINCT(tbl_total_koef.kode_material) as kd_material,
					simpro_tbl_detail_material.detail_material_nama,
					simpro_tbl_detail_material.detail_material_satuan,
				...
				FROM (				
					...
					INNER JOIN simpro_costogo_analisa_item_apek ON simpro_costogo_analisa_item_apek.kode_analisa = tbl_asat_apek.parent_kode_analisa AND simpro_costogo_analisa_item_apek.id_proyek = '%d'
					INNER JOIN simpro_costogo_item_tree ON simpro_costogo_item_tree.id_proyek = simpro_costogo_analisa_item_apek.id_proyek AND simpro_costogo_item_tree.kode_tree = simpro_costogo_analisa_item_apek.kode_tree					
				) as tbl_total_koef 
				INNER JOIN (
					SELECT 
	...
	function costogo_costogoa($idproyek,$tgl_rab)
	{
		$sql = sprintf("		
				SELECT 
					DISTINCT(tbl_total_koef.kode_material) as kd_material,
				...
				FROM (				
					...
					INNER JOIN simpro_costogo_analisa_item_apek ON simpro_costogo_analisa_item_apek.kode_analisa = tbl_asat_apek.parent_kode_analisa AND simpro_costogo_analisa_item_apek.id_proyek = '%d' and simpro_costogo_analisa_item_apek.tanggal_kendali = '$tgl_rab'
					INNER JOIN simpro_costogo_item_tree ON simpro_costogo_item_tree.id_proyek = simpro_costogo_analisa_item_apek.id_proyek AND simpro_costogo_item_tree.kode_tree = simpro_costogo_analisa_item_apek.kode_tree					
				) as tbl_total_koef 
				INNER JOIN (
					SELECT 
