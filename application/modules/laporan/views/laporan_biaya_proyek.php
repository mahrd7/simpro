...
if(isset($_POST['inp_bulan'])){
    $bulan_ = $_POST['inp_bulan'];
}
else if($bulan_==""){
    $bulan_=date('m');
}
...
if(isset($_POST['inp_tahun'])){
    $tahun_ = $_POST['inp_tahun'];
    }
else if($tahun_==""){
    $tahun_=date('Y');
}else{
...
<body>
	<div class="titlegroup">Laporan Biaya Proyek (LBP-01) Bulan <?php echo $bl[$bln] ." ". $tahun_; ?></div>
	<div class="headgroup">
		<div class="deskripsi">Peningkatan Jalan Beton Salubatu - Aralle II Mamasa Sulawesi Barat</div>
		
        <?php
        echo "<form method=post action=?mode=lihat&tahun_=$tahun_&bulan_=$bulan_>";
        ?>    
			Periode :
			<?php
            echo combo_bulan("inp_bulan",$bulan_).combo_tanggal("inp_tahun",$tahun_,2000,2015);
            ?>		
			<input type="submit" value="Go >">	
        
        <div class="actions">
            <a href="<?=base_url();?>laporan/printed_lbp?tahun_=<?=$tahun_;?>&bulan_=<?=$bulan_;?>" title="Print"><span class="icon-print">Print</span></a>
        </div>								
		</form>
	...
	<table id="cashflow-grid">
		<thead>
			<tr>
				...
				<th rowspan="2"><?php echo $JUDUL; ?></th>
				<th rowspan="2">SD.Bulan <?php echo $bl[$blnlalu]; ?></th>
				<th colspan="7">Realisasi Bulan <?php echo $bl[$bln]; ?></th>
				<th rowspan="2">Rencana Bulan <br /><?php echo $bl[$blndpn]; ?></th>
